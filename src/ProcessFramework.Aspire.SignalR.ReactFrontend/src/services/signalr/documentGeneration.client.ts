...
export class SignalRDocumentationGenerationClient {
    ...
    async joinProcessGroup(processId: string): Promise<void> {
        await this.connection.invoke("JoinProcessGroup", processId);
    }

    async leaveProcessGroup(processId: string): Promise<void> {
        await this.connection.invoke("LeaveProcessGroup", processId);
    }
